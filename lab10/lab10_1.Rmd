...
ggplot(data=iris, #specify the data
       mapping=aes(x=Species, y=Petal.Length)) #map the aesthetics, x=(what you want on x axis), y=(what you want on y axis)
```
...
```{r}
glimpse(homerange) #categorical, character variables and numerical variables
```
...
```{r}
miss_var_summary(homerange) #number of NA's
```
...
ggplot(data=homerange, mapping=aes(x=log10_mass, y=log10_hra))+
  geom_jitter() #adds some random noise to separate things a bit
```
...
ggplot(data=homerange, mapping=aes(x=log10_mass, y=log10_hra))+
  geom_point()+
  geom_smooth(method=lm, se=T) #add a regression line; another layer (these plots are built in layers)
```
...
ggplot(data = homerange,
       mapping = aes(x = log10_preymass, y = log10_hra)) + 
         geom_point() +
  geom_smooth(method = lm, se = T)
```

Ggplot removes NA values by default (see warning message)? Can remove the warning by inputting na.rm = T in geom_point()     
...
```{r}
homerange %>% #avoid saying "data="
  ggplot(aes(x=trophic_guild)) + #can even just remove the "x=" because it is implied in this case
  geom_bar() #good for counts; does not work with continuous data
```
...
homerange %>% 
  filter(family=="salmonidae") %>%
  select(common_name, log10_mass) %>% 
  ggplot(aes(x=common_name, y=log10_mass))+ #notice the switch in x and y
  geom_col()
```
...
homerange %>% 
  ...
  geom_col() #categorical + numerical data; if you only have counts, use geom_bar()
```
