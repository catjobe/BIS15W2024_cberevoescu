...
penguins %>% 
  filter(sex != "NA") %>% 
  group_by(sex) %>% 
  summarize(average_bill_length = mean(bill_length_mm, na.rm = T)) %>% 
...
homerange %>% 
  ...
  summarize(min_mass = min(log10.mass, na.rm = T),
            max_mass = max(log10.mass, na.rm = T),
            mean_mass = mean(log10.mass, na.rm = T))
```
