...
```{r}
life_history %>% 
  ggplot(aes(x = order)) + #don't need to specify y bc y will be by default a count
  geom_bar() +
  coord_flip()
```
...
life_history %>% 
  ...
  ggplot(aes(x = order, y = mean_mass)) + #you oftentimes don't have to specify x and y, it will be done by default
  geom_col() +
  coord_flip()
```
...
life_history %>% 
  group_by(order) %>% 
  summarise(mean_mass = mean(mass, na.rm = T)) %>% 
  ggplot(aes(x = reorder(order, mean_mass), y = mean_mass)) + #this allows use to order the x axis by mean_mass
  geom_col() +
  coord_flip() +
  scale_y_log10()
```
...
elephants %>% 
  ...
  labs(title = "Elephant Age vs. Height",
       x="Age (years)",
       y="Height (cm)") #adding a title and naming the x and y axes
```
...
elephants %>% 
  ...
  labs(title="Elephant Age vs. Height", #adds a title
       x="Age (years)", #naming the x axis
       y="Height (cm)") + #naming the y axis
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5)) #moving and resizing labels to desired size and location
```
