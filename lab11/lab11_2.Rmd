...
```{r}
p <- homerange %>% 
  ggplot(aes(x= log10.mass, y= log10.hra)) #storing base plot, without specifiying a geom, as an object
```
...
```{r}
p + geom_point(aes(color = thermoregulation), size = 1.5) #need to go into an aesthetic wrapper; for scatterplots color is the same as fill in barplots
```
...
```{r}
homerange %>% 
  ggplot(aes(x = taxon, fill = trophic.guild)) + #this provides more information on the plot
    geom_bar() +
  coord_flip() +
  labs(title = "Observations by Taxon",
       x = "Taxonomic Group")
...
```{r}
penguins %>% 
  filter(sex != "NA") %>% #this removes the NA's from the variable sex; the NA's might have been juvenile birds on the islands that were not sexed
...
```{r}
homerange %>% 
  ggplot(aes(x = class, y = log10.mass, group = taxon)) + #no color if group by taxon
  geom_boxplot()
```
...
```{r}
homerange %>% 
  ggplot(aes(x = class, y = log10.mass, group = taxon, fill = taxon)) + #get color if fill by taxon (but does the same thing as group)
  geom_boxplot(alpha = 0.4) + #adding some transparency to better see the background
  labs(title = "Mass by Class",
       fill = "Taxon")
```
