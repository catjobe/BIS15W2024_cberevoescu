...
```{r}
treatment+control
#using tab complete prevents spelling errors
```
...
```{r}
all_top_speed <- sum(venom_GT,chiron,veyron)
all_top_speed/3
```
...
```{r}
mean(venom_GT+chiron+veyron) #DOESN'T work, the mean function sees this as one object
```

```{r}
cars <- c(venom_GT, chiron, veyron) #c is Concatenate
mean(cars) #Correct
```

```{r}
mean(c(venom_GT, chiron, veyron)) #Correct
```


```{r}
mean(venom_GT,chiron,veyron) #Uses different type of mean
```
...
If an NA is included in the object, functions in R will let you know of their presence and not run. If NAs are presence, R functions will not conduct the functions that you want the to do.

4. Recalculate the mean using the following code chunk. Why is the useful? 

This is useful because it will allow different R functions to actually do their functions on the data.
