...
```{r}
p + theme_linedraw() + #one of the built in themes in R
...
p+theme_linedraw()+
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 60, hjust=1))+ #adjusting the legend
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL, #the null just removes the label from the x axis
       y= "n",
       fill= "Trophic Guild") 
```
...
```{r}
my_palette <- paletteer_d("ggsci::nrc_npg") #store your palette; have to add package and palette
new_palette <- paletteer_d("futurevisions::atomic_orange")
```
...
homerange %>% 
  ggplot(aes(x=taxon, y=log10.mass)) +
  geom_boxplot() +
  facet_grid(.~trophic.guild) + #~ means all options within the variable trophic.guild
  theme(axis.text.x = element_text(angle = 60, hjust=1))
```
...
homerange %>% 
  ggplot(aes(x=taxon, y=log10.mass)) +
  geom_boxplot() +
  facet_grid(trophic.guild~thermoregulation) + #~ means all options within the variable trophic.guild
  theme(axis.text.x = element_text(angle = 60, hjust=1))
```
