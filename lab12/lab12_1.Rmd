...
```{r}
homerange %>% 
  ggplot(aes(x = class, y = log10.mass, group = taxon)) + #group doesn't color in the boxes; grouping by a different categorical variable than you plotted
  geom_boxplot() +
  coord_flip()
```
...
## Line plots
Line plots are great when you need to show changes over time. Here we look at the number of samples for species DM and DS over the years represented in the `deserts` data. This takes some careful thought- we want to know how sampling has changed over time for these two species. (example of a hard problem on midterm 2)    

Let's start by making a clear x and y so we know what we are going to plot.
```{r}
deserts %>% 
  filter(species_id == "DM" | species_id == "DS") %>% 
  mutate(year = as.factor(year)) %>% #trick for the hw; bc if leave year as numeric, R thinks you want to do math with it but that's not what you do with years, they represent individual sampling points in our data, not continuous data
  group_by(year, species_id) %>% 
  summarize(n = n(), .groups = 'keep') %>% #.groups cancels the error
  pivot_wider(names_from = species_id, values_from = n) #easier to read
```
...
homerange %>% 
  ...
  geom_histogram(bins = 50)+ #we can adjust the number of bins with the bins argument
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_histogram(color = "lavenderblush4", fill = "lavenderblush2", bins=10) + #use `color` to specify the color of the outline and `fill` to specify the color of the fill
  labs(title = "Distribution of Body Mass")
```
...
```{r}
homerange %>% 
  ggplot(aes(x = log10.mass)) + #only need to specify one variable, the x
...
homerange %>% 
  ggplot(aes(x=log10.mass)) +
  geom_histogram(aes(y = after_stat(density)), fill = "deepskyblue3", alpha = 0.4, color = "deepskyblue4")+
  geom_density(color = "turquoise1")+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ggplot(aes(x = log10.hra)) +
  geom_histogram(aes(y = after_stat(density)), fill = "lavenderblush", alpha = 0.75, color = "lavenderblush3") +
  geom_density(color = "lavenderblush4") +
...
```{r}
summary(homerange$log10.mass) #this is from base R
```
...
homerange %>% 
  mutate(mass_category = case_when(log10.mass <= 1.75 ~ "small",
                                   ...
                                   log10.mass > 2.75 ~ "large")) #adding categories to make a continuous variable categorical
```
...
homerange %>% 
  mutate(mass_category = case_when(log10.mass <= 1.75 ~ "small",
                                   log10.mass > 1.75 & log10.mass <= 2.75 ~ "medium",
                                   log10.mass > 2.75 ~ "large")) %>% #adding categories
  ggplot(aes(x = mass_category, fill = trophic.guild)) +
  geom_bar(position = "dodge") #making side by side
```
...
```{r}
quartiles <- quantcut(homerange$log10.hra) #from gtools package
table(quartiles)
```
...
homerange %>% 
  ...
  geom_bar(position = "dodge", alpha = 0.6, color = "grey")
```
