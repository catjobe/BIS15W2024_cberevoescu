...
```{r}
mammals %>% #very clean, can do this all in one go
...
fish %>%  #work with the fish data 
  ...
  filter(lakeid == "AL" | lakeid == "AR") %>% #filter out for only these lakes
  filter(between(radii_length_mm, 2, 4)) %>% #filter out data only between 2 and 4
  arrange(desc(radii_length_mm)) #sort to make easier to read; descending order
```
...
```{r}
fish %>% #extracting information from the fish data
  filter(fish_id == 300) %>% #selecting fish with the id 300
  select(radii_length_mm, scalelength) %>% #selecting only the columns we are interested in
  arrange(radii_length_mm) #arranging the data so it is easier to read
```
