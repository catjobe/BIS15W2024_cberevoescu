...
```{r}
glimpse(fish) #6 columns, 6 variables
```
...
```{r}
filter(fish, lakeid == "LJ") #pulls out all the observations for lakeid being LJ
```
...
```{r}
filter(fish, length > 400, (scalelength > 11 | radii_length_mm > 8)) #nesting conditions
```
...
```{r}
bovidae <- filter(mammals, mass > 450000, family == "Bovidae")
bovidae
```
...
mammals4 #a pain to do this
```


```{r}
primates <- filter(mammals, genus == "Lophocebus" | genus == "Erythrocebus" | genus == "Macaca")
primates <- select(primates, "order", "genus", "species", "mass", "gestation", "newborn")
primates
```

```{r}
summary(primates$newborn)
filter(primates, newborn == 389.5)
```

```{r}
minprimate <- min(primates$newborn)
filter(primates, newborn == minprimate)
```
