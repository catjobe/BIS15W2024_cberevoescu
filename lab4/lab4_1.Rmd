...
```{r}
select(fish, "fish_id", "length") #can copy and paste to avoid misspelling
```

```{r}
fish_subset <- select(fish, "length", "fish_id") #reversed
```
...
```{r}
select(fish, matches("a.+er")) #handy bit o' code for the project hint hint
```
...
```{r}
mammal_life_histories <- read.csv("data/mammal_lifehistories_v2.csv")
```

1. Use one or more of your favorite functions to assess the structure of the data.  

```{r}
glimpse(mammal_life_histories)
```

2. Are there any NAs? Are you sure? Try taking an average of `max. life` as a test.  

```{r}
anyNA(mammal_life_histories)
```

There are no NAs based off of the anyNA(function)

```{r}
mean(mammal_life_histories$max..life) #why is the value negative?
```

```{r}
summary(mammal_life_histories)
```

3. What are the names of the columns in the `mammals` data?   

```{r}
names(mammal_life_histories)
```

4. Rename any columns that have capitol letters or punctuation issues.  

```{r}
mammals_new <- rename(mammal_life_histories, genus="Genus", wean_mass="wean.mass", afr="AFR", max_life="max..life", litter_size="litter.size", litters_year="litters.year")
mammal_life_histories
```
...
```{r}
select(mammal_life_histories, contains("mass"))
```
...
```{r}
select_if(mammals_new, is.character) #works the same!
```
...
```{r}
select_all(mammal_life_histories, tolower)
```

```{r}
library(janitor) #top secret trick
mammal_life_histories
names(mammal_life_histories)
```

```{r}
clean_names(mammal_life_histories) #cleans it all FOR you! VERY handy
```
