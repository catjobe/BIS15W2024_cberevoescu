...
```{r}
mammals <- read_csv("data/mammal_lifehistories_v2.csv") %>% clean_names() #attaching read names with a pipe to clean the data
```
...
```{r}
fish %>% 
  mutate(length_half = length/2) %>% #adding a new column that is half length of each fish, the new variable is always added at the very end
  select(fish_id, length, length_half) #selecting only fish_id, length, length_half columns
```
...
```{r}
mammals %>% #since this data is not put into a new object, it is not saved as a new object (try to not create new objects for each data manipulation/function)
  mutate_all(tolower) #cleaning up the data, all of the observations cleaned up
```
...
```{r}
mammals %>% 
  mutate(across(c("order", "family"), tolower)) #cleaning up the data, only certain variables + observations
```
...
```{r}
mammals %>%
  select(family, genus, species, max_life) %>% #can run after each line to make sure that each line worked
  mutate(max_life_new = ifelse(max_life == -999.00, NA, max_life)) %>% #can check if NAs by doing arrange(max_life_new)
  arrange(desc(max_life_new))
```

The animal with the oldest known lifespan is Baleanoptera physalus with a max lifespan of 1368 years.

```{r}
filter(mammals, max_life == 1368)
```
