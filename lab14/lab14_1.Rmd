...
```{r, eval=F, echo=T}
runApp("plot_iris.R") #using built in data; allows users to go in and select variables to plot (changes plot in response to that)
```
...
2.  The `server` function is the part of the app which takes the values of the user inputs, performs calculations and/or makes plots, and prepares the outputs for display; i.e. this controls how your app works. (does the work)
...
server <- function(input, output, session) { #part 2
  
}

shinyApp(ui, server) #part 3
```
...
```{r, eval=F, echo=T}
ui <- fluidPage(
    selectInput("x", "Select X Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), selected = "bill_length_mm") #referring to the drop down menu
)
...
```{r, eval=F, echo=T}
ui <- fluidPage( #keep a lot of spaces between code chunks to avoid confusion (be careful to be clear - even the absence of one comma can break the app, and it can be difficult to troubleshoot)
...
penguins %>% 
  ...
  geom_point() #always build the desired plot at least once in advance
```
...
server <- function(input, output) {
  ...
  output$plot <- renderPlot({
      ...
      ggplot(data = penguins, aes_string(x = input$x, y = input$y, color = "species")) + #creating the plot, not using pipes
      geom_point() +
      theme_light(base_size = 18)
    
  }) #curly brackets closing render plot
  
} #curly brackets closing the server function
...
ui <- fluidPage(
   ...
   selectInput("x", "Select X Variable", choices = c("locomotion"), 
    ...
    selectInput("y", "Select Fill Variable", choices = c("trophic.guild", "thermoregulation"), 
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
      ...
      ggplot(data = homerange, aes_string(x = input$x, fill = input$y)) +
      geom_bar(position = "dodge") +
      theme_light(base_size = 18) +
      labs(title = "Locomotion Barplot",
           x = "Locomotion",
           y = "Count") +
...
ui <- fluidPage(
   ...
   radioButtons("x", "Select Fill Variable", choices = c("trophic.guild", "thermoregulation"), 
    ...
    plotOutput("plot") #not specifying the size, although you could
)
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({ #make a plot
      ...
      ggplot(data = homerange, aes_string(x = "locomotion", fill = input$x)) + #making the reactive part the fill variable
      geom_bar(position = "dodge", alpha = 0.75, color = "lavender") +
...
} #closes server functin

shinyApp(ui, server)
) #if the app is broken or not working, can troubleshoot by removing elements (find where it is breaking); make sure everything is clearly spaced out, that the brackets are in the right place (can annotate the brackets for clarity) and you know where they are open and closed.
```
