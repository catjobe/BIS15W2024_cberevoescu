...
During lab 2, you learned how to work with vectors and data matrices. Today we will organize the vectors into a new type of data structure called a **data frame**. The data frame is the most common way to organize data within R. You can think of a data frame as similar to a spreadsheet. 
...
```{r}
hbirds <- data.frame(Sex, Length, Weight) #uppercase variables
hbirds
```
...
```{r}
hbirds <- data.frame(sex=Sex, length=Length, weight_g=Weight) #save to data frame
hbirds
```
...
```{r}
hsprings_og <- data.frame(temp, name, spring) #can reorder columns here
hsprings_og
```
...
```{r}
hsprings <- data.frame(temp_C=temp, scientist=name, spring) #overwriting to keep the data frame name (can not always be the best practice, be careful when overwriting)
hsprings  #to do this, must rebuild dataframe, but specify a different name for the variable
```
...
```{r}
hsprings$depth_ft <- c(4.15, 4.13, 4.12, 3.21, 3.23, 3.20, 5.67, 5.65, 5.66) #a new variable
hsprings #overwrote previous version
```

Another method:

```{r}
hsprings_og_edit <- data.frame(temp_C=temp, name, spring)
hsprings_og_edit
depth_ft <- c(4.15, 4.13, 4.12, 3.21, 3.23, 3.20, 5.67, 5.65, 5.66)
hsprings_method <- cbind(hsprings_og_edit, depth_ft)
hsprings_method
```
