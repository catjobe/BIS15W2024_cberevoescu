...
```{r}
class(hot_springs$scientist) #class of the scientist column; a character can be any type
```

```{r}
hot_springs$scientist <- as.factor(hot_springs$scientist) #factor is when you have repeated types (discrete classes/categories of characters)
hot_springs$spring <- as.factor(hot_springs$spring)
```

```{r}
levels(hot_springs$scientist) #tells you how many factors you have in a variable
```

```{r}
levels(hot_springs$spring) #doesn't work until changing spring from character (see above)
```
...
```{r}
setwd("/Users/catjobe/Desktop/BIS15W2024_cberevoescu/lab3") #can change the directory using setwd() so that RStudio will see the data
fish <- read_csv("data/Gaeta_etal_CLC_data.csv") #or can specify directory directly
```
...
```{r}
glimpse(fish) #gives snapshot of what fish looks like
```
...
```{r}
summary(fish) #gives basic stats, cannot give information for non-numeric data
```

`glimpse()` is another useful summary function.   

```{r}
glimpse(fish) #clean results
```
...
```{r}
names(fish) #gives names of the variables
```

`head()` prints the first n rows of the data frame.  

```{r}
head(fish) #really important one
```
...
```{r}
#table(fish) #doesn't always work, can get too big, may have to specify what you want to look at
table(fish$lakeid)
```
...
```{r}
#View(fish) 
#same as clicking spreadsheet button in top right panel
```
...
```{r}
little_fish <- filter(fish, length<=100) #can't do this in excel
little_fish
```
...
```{r}
str(mammals) #also provides information on the data class of each variable
```
...
```{r}
table(mammals$order) #counts of mammal order
```

```{r}
table(mammals$family) #counts of family
```

```{r}
table(mammals$Genus) #counts of genus
```

```{r}
#table(mammals) 
#mammals data set is too large to make a table with
```
