...
```{r}
penguins %>% 
  filter(!is.na(body_mass_g)) %>% #pull out all the observations that actually have a number, and leave the NAs behind
  group_by(island) %>% 
  summarize(mean_body_mass=mean(body_mass_g),
            n=n()) #this just provides the number/counts of observations for the variable
```
...
```{r}
penguins %>% 
  tabyl(island, species) #part of janitors package
```
...
2. For some penguins, their sex is listed as NA. Where do these penguins occur? (problems recording the data)   
...
```{r}
penguins %>%
  summarize(across(contains("mm"), \(x) mean(x, na.rm = TRUE))) #FIX the error message from above with this code
```
