...
```{r}
names(msleep)
msleep <- msleep #can do this to visualize the data in RStudio
```

```{r}
msleep24 <- msleep %>% 
  mutate(sleep_total_24 = sleep_total / 24) %>% #creating a new column called sleep_total_24
  select(name, vore, sleep_total_24, sleep_total) %>% #to check if the mutate() 
...
```{r}
summary(msleep24) #the max value is 0.29167, which is less than 0.3, so the filter function worked; the first two columns don't really help
```
...
`n_distinct()` is a very handy way of cleanly presenting the number of distinct observations. Here we show the number of distinct genera over 100 in body weight.  NOTE: will be on exam!! Very handy function
...
```{r}
msleep %>% 
  summarize(n_genera=n_distinct(genus)) #this is going to count the number of UNIQUE genera in msleep
```
...
The function still runs (it doesn't error out), but it cannot calculate the means for categories in which there are any NAs (no data available).   
...
3. Try running the code again, but this time add `na.rm=TRUE`. What is the problem with Cetacea (the whales)? Compare this to Carnivora.  

```{r}
mean_brainwts <- msleep %>% 
  group_by(order) %>% 
  summarize(mean_brainwt=mean(brainwt, na.rm = TRUE))
mean_brainwts
```

I would guess, based on comparing these two orders, that there is no brain weight data (no one has done it, maybe its hard to do) for the Cetacea:   
