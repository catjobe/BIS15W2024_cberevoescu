...
```{r}
?penguins #information on the penguins data
penguins <- penguins
```
...
penguins %>% 
  group_by(island) %>% 
  summarize(mean_body_mass_g=mean(body_mass_g, na.rm=T)) # remember to remove those NA's! put na.rm() in the RIGHT place!
```

What if we are interested in the number of observations (penguins) by species and island?    
```{r}
penguins %>% #can save this output as a new object or dataframe
  group_by(island, species) %>% #grouping by two categorical variables at the same time, if change the order the order of the columns will be different, but the data will be the same
...
```{r}
penguins %>% 
  tabyl(island, species) #tabyl() is part of the janitor package, which is NOT part of tidyverse
```
...
penguins %>% 
  ...
  summarise(mean_bill_length = mean(bill_length_mm, na.rm = T),
            ...
            n_samples = n()) #this is too long and prone to mistakes
```
...
The penguin species with the largest mean bill length is the Chinstrap species (48.83382 mm).    
