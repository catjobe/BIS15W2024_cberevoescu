...
```{r}
life_history <- read_csv("data/mammal_lifehistories_v3.csv", na= c("NA", " ", ".", "-999", "-999.0", "not measured")) %>% clean_names() #can only really do this when you have found the NAs, spaces, dots, etc (other placeholders); need to know how the NA's are represented in the data
...
```{r}
naniar::miss_var_summary(life_history) #naniar:: just tells the software to use the naniar package
#miss_var_summary() tells you number and percent of missing (NA) data
```

Notice that `max_life` has no NA's. Does that make sense in the context of this data?
```{r}
hist(life_history$max_life) #this shows that 0s are used as placeholders (NAs) in this data set
```
...
life_history %>% 
  replace_with_na(replace = list(newborn = "not measured", 
                                 ...
                                 gestation= -999, 
                                 mass= -999)) %>% 
miss_var_summary() #have to be very specific
#makes replaced of NA's specific to a variable
```
...
```{r}
cites_wildlife <- read_csv("data/cites.csv") %>% clean_names()
```
...
cites_wildlife %>%
  ...
  miss_var_summary() %>% #how much data is actually missing
...
life_history_advance <- 
  readr::read_csv(file = "data/mammal_lifehistories_v3.csv", 
                  na = c("NA", " ", ".", "-999")) #all NA, blank spaces, .,and -999 are treated as NA; should only be done if you understand data in the data set you are working with - don't use this blindly
```
