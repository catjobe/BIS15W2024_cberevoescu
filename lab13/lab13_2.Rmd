...
lat <- c(34.67, 41.80)
long <- c(-124.1, -115.5)
bbox <- make_bbox(long, lat, f = 0.03) #f is the fraction of the bounding box to add to the range; the program is very sensitive to this value, recommended to start with 0.03
```

Let's get a base map for our bounding box area. There are several different map [styles](https://docs.stadiamaps.com/themes/).   

```{r}
map1 <- get_stadiamap(bbox, maptype = "stamen_terrain", zoom=7) #also very sensitive to zoom, keep this at zoom = 7
```
...
```{r}
ggmap(map1) + 
  geom_point(data = spiders, aes(longitude, latitude), size=0.4) + #modified ggplot syntax; CANNOT use pipes in this; be very careful with the syntax
...
```{r}
ggmap(map2) + 
  geom_point(data = sharks_dups, aes(longitude, latitude, color = injury), size = 0.4) + #can use the aesthetics we have used previously
...
```{r}
ggmap(map3) +
  geom_point(data = shark_dups_fatal, aes(longitude, latitude), size = 0.8) + 
