...
```{r}
files <- list.files(path = "data/spiders", pattern = ".csv", full.names = TRUE) #manage import of many files from one project
files
```
...
```{r, message = FALSE}
spider_list <- lapply(files, read_csv) #"list apply"; looks over the files in files, and it applies read_csv to each of them.
```
...
```{r}
butte <- spider_list[["Butte"]] #store as temporary object in environment panel, can now use data normally
```
...
`inner_join` - only matches observations for which ALL of the columns are present for a certain ID.  
...
`left_join` - matches all the IDs from the left table (absolute).      
...
`right_join` - matches all the IDs from the right table (absolute).   
...
`full_join` - matches all the IDs, shows missing data where appropriate.   
...
```{r}
anti_exampleDF <- anti_join(table_A, table_B, by="customer_ID") #can be used for troubleshooting
anti_exampleDF
```
...
```{r}
spiders_with_locs <- left_join(spiders_all, spiders_locs, by = "Accession") #can use full_join() if there are no missing values between the two data sets
spiders_with_locs
...
#glimpse(spiders_with_locs) #not interpreting date in the correct way
```
...
```{r}
as.Date(spiders_with_locs$Date)
```

```{r}
dmy(spiders_with_locs$Date) #be careful, the authors could be using European conventions
```
...
#5 normally shown conventions for date formats
```
